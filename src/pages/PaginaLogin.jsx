...
function PaginaLogin() {
  ...
  useEffect(() => {
    if (estaAutenticado) {
      navegacao('/endereco', { replace: true });
    }
  ...
  const verificarCodigo = async (e) => {
    ...
    try {
      if (metodoLogin === 'email') {
        ...
        // Primeiro verifica o código com o Twilio
        await verifyCode(telefone, codigo);
        
        // Após verificar o código, cria ou atualiza o usuário no Supabase
        const { data: { user }, error: signUpError } = await supabase.auth.signUp({
          email: `${numeroFormatado.replace(/\D/g, '')}@phone.temp`,
          password: `${codigo}${Date.now()}`, // Senha única e segura
  ...
  const aoLoginSocial = async (provedor) => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          redirectTo: `${window.location.origin}/endereco`, // Deve estar cadastrado no Facebook e Supabase
        },
      });
